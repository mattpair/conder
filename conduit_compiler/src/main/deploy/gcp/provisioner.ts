...
class GoogleCloudProvisioner {
    ...
    async deployOnToCluster(cluster: container.protos.google.container.v1.ICluster, serviceName: string, contName: string): Promise<string> {
        ...
        kc.setCurrentContext(cluster.name)
...
export async function create(manifest: FunctionResolved.Manifest, containerName: string): Promise<string> {
    const client = new container.v1.ClusterManagerClient()
    const projectId = await client.getProjectId()
    const zone =  "us-west1-a"
    const clusterId = manifest.service.name
    ...
    return provisioner.createCluster().then((cluster) => {
        fs.writeFileSync(".deploy/receipt.json", cluster.name)
...
export async function destroy() {
    ...
    const projectId = await client.getProjectId()
    const zone =  "us-west1-a"
    const clusterId = fs.readFileSync(".deploy/receipt.json", {encoding: "utf-8"})
    const provisioner =  new GoogleCloudProvisioner(client, projectId, zone, clusterId)
